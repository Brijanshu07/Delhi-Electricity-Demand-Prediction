...
def load_model():
    ...
    print("Loaded model with features:", booster.feature_names)
    return True
...
def build_features_for_datetime(dt, temp, rhum):
    hour = dt.hour
    month = dt.month
    day = dt.day
    day_of_week = dt.dayofweek
...
def predict_from_features(feats: dict) -> float:
    X = pd.DataFrame([feats], columns=booster.feature_names).fillna(0)
    dmat = xgb.DMatrix(X, feature_names=booster.feature_names)
    return float(booster.predict(dmat)[0])

# ===================== SERIES =====================

def get_next_n_hours(n: int):
    if model is None:
        return []
    ...
    for i in range(n):
        t = now + pd.Timedelta(hours=i)
        temp = 30 + 5 * np.sin(2 * np.pi * t.hour / 24)
        rhum = 55

        feats = build_features_for_datetime(t, temp, rhum)
        base_pred = predict_from_features(feats)
...
def api_series():
    return {
        ...
        "next_10_hours": get_next_n_hours(10),
    }

@app.post("/api/predict", response_model=PredictResponse)
def api_predict(req: PredictRequest):
    feats = build_features_for_datetime(
        pd.to_datetime(req.date),
        req.temp,
        req.humidity
    )

    pred = predict_from_features(feats) * CALIBRATION_FACTOR
